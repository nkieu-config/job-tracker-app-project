...
**4 AI features · 11 app pages · 8-table Postgres schema · 159 tests + a 3-suite AI eval harness · ~9k lines of strict TypeScript**
...
159 tests across two Vitest projects — a **Node** project for server code (ownership scoping of every Server Action, the resume upload's blob lifecycle including compensating deletes, rate limiting, embedding batch splitting) and a **jsdom** project for components (the streaming UI's save/discard rules, accessibility invariants of the drag-and-drop board). Four are rate-limit integration tests that run against a real Postgres and skip when no database is reachable. Security-critical modules are pinned to **100% coverage thresholds** in CI.
